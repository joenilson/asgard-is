 ...
 */
Ext.define('Asgard.lib.grid.ownersprofile',{
    extend: 'Asgard.lib.GridPanel',
    alias: 'widget.gridownersprofile',
    autoShow: true,
    autoDestroy: true,
    autoScroll: true,
    border: true,
    ...
    titleNewAuditPlan: 'Add new Owner Profile',
    titleEditAuditPlan: 'Edit Owner Profile data',
    ...
    initComponent: function(){
        ...
        this.columns =  {
            ...
            items: [
                ...
                {text: this.descText, flex: 3, sortable: true, hidden: false, dataIndex: 'profile_desc', filter: true},
    ...
    showDocument: function(obj, id, component, icon, event, record){
        if(record.data.profile_desc!==''){
            var windowDoc = this.createWindow();
            //windowDoc.removeAll();
            windowDoc.setTitle(record.data.profile_desc);
            var documentFile = Ext.create('Ext.ux.panel.PDF', {
                ...
                src      : record.data.profile_file, // URL to the PDF - Same Domain or Server with CORS Support
