...
class IndexController extends AbstractActionController
{
    ...
    public function getmtmAction(){
        ...
        $yearParams = $request->getQuery('year');
        $sql = $this->getMTMTable();
        $listDocuments = $sql->getObjectByCCLYByLang($companyParams,$countryParams,$locationParams,$yearParams,$lang);
    ...
    public function getmtmtypeAction(){
        $userPrefs = $this->getServiceLocator()->get('userPreferences');
        $lang=$userPrefs[0]['lang'];
        $request = $this->getRequest();
        $sql = $this->getMTMTypeTable();
        $dataRequirement = $sql->getObjectList($lang);
        if($dataRequirement){
            $dataResult['success']=true;
    ...
    public function addmtmAction(){
        ...
        $country = $request->getPost('countriesCombo');
        $location = $request->getPost('locationsCombo');
        $type = $request->getPost('gentypeCombo');
        $year = $request->getPost('yearsCombo');
        $description = $request->getPost('description');
        $files =  $request->getFiles()->toArray();
        $id = $request->getPost('mtm_id');
        $date_creation = \date('Y-m-d h:i:s');
    ...
    public function removemtmAction(){
        $userData = $this->getServiceLocator()->get('userSessionData');
        $request = $this->getRequest();
        $object_id = $request->getPost('id');
        $company = $request->getPost('company');
        ...
        $type = $request->getPost('type');
        $sql = $this->getMTMTable();
        $objectData = $sql->getObjectByCCLIT($company,$country,$location,$object_id,$type);
        $dataRemove = array();
        $dataResult = array();
        $dataResult['success'] = false;
        if(count($objectData)>0){
    ...
    public function getohrtypesAction(){
        $userPrefs = $this->getServiceLocator()->get('userPreferences');
        $lang = $userPrefs[0]['lang'];
        $sql = $this->getOHRTypeTable();
        $listDocuments = $sql->getOHRTypeList($lang);
    ...
    public function addohrAction(){
        ...
        $year = $request->getPost('yearsCombo');
        $description = $request->getPost('description');
        $type = $request->getPost('ohrtypeCombo');
        $files =  $request->getFiles()->toArray();
        $id = $request->getPost('ohr_id');
        $date_creation = \date('Y-m-d h:i:s');
        ...
        if(isset($newId) AND $valid->isValid($files['ohr_file'])){
            $fileName = "ohr_{$company}_{$country}_{$location}_".$newId.".pdf";
            $this->savefile('library/ohr', $files['ohr_file'], $fileName, false, null);
            $sql->update(array('filename'=>'library/ohr/'.$fileName),array('id'=>$newId));
        }
        return new JsonModel($dataResult);
    }
    
    public function removeohrAction(){
        $userData = $this->getServiceLocator()->get('userSessionData');
        $request = $this->getRequest();
        $object_id = $request->getPost('id');
        $company = $request->getPost('company');
    ...
    public function formohrAction(){
        $userData = $this->getServiceLocator()->get('userSessionData');
        $request = $this->getRequest();
        $object_id = $request->getPost('id');
        $type_id = $request->getPost('type');
        $company = $request->getPost('company');
        $country = $request->getPost('country');
        $location = $request->getPost('location');
        $sql = $this->getOHRTable();
        $objectData = $sql->getObjectByCCLIT($company,$country,$location,$object_id,$type_id);
        if($objectData){
            $dataDocuments['description'] = $objectData[0]['description'];
            $dataDocuments['year_date'] = $objectData[0]['year_date'];
            $dataDocuments['companiesCombo'] = $objectData[0]['company'];
            $dataDocuments['countriesCombo'] = $objectData[0]['country'];
            $dataDocuments['locationsCombo'] = $objectData[0]['location'];
            $dataDocuments['ohrtypeCombo'] = $objectData[0]['id_type'];
            $dataResult['success']=true;
            $dataResult['data']=$dataDocuments;
        }else{
            $dataResult['success']=false;
...
//        "activity_desc":"Revis\u00e3o inicial e rotineira de \u00e1gua em tanques"
    ...
    private function getMTMTable()
    {
    	if (!$this->mtmTable) {
            $sm = $this->getServiceLocator();
            $this->mtmTable = $sm->get('IMS\Model\MTMTable');
    	}
    	return $this->mtmTable;
    }
    
    private function getMTMTypeTable()
    {
    	if (!$this->mtmtypeTable) {
            $sm = $this->getServiceLocator();
            $this->mtmtypeTable = $sm->get('IMS\Model\MTMTypeTable');
    	}
    	return $this->mtmtypeTable;
    }
    ...
    private function getOHRTable()
    {
    	if (!$this->ohrTable) {
            $sm = $this->getServiceLocator();
            $this->ohrTable = $sm->get('IMS\Model\OHRTable');
    	}
    	return $this->ohrTable;
    }
    
    private function getOHRTypeTable()
    {
    	if (!$this->ohrtypeTable) {
            $sm = $this->getServiceLocator();
            $this->ohrtypeTable = $sm->get('IMS\Model\OHRTypeTable');
    	}
    	return $this->ohrtypeTable;
    }
