...
class SimulationAlbumsTable extends AbstractTableGateway {

    protected $table_name = 'simulation_albums';
    protected $photos_table_name = 'simulation_photos';
    ...
    public function getObjectByCCL($company,$country,$location)
    {
        $row = $this->select(function (Select $select) use ($company,$country,$location){
            $select->where(array('company'=>(string) $company,
                                ...
                                'status'=>'A'));
            $select->order('date_simulation DESC');
        });
        if (!$row)
            return false;
        return $row->toArray();
    ...
    public function save(SimulationAlbums $object)
    {
        $data = array(
        ...
        if (!$this->getObjectByCCLId($company,$country,$location,$id)) {
            ...
            throw new \Exception('company or country or location or id in object Simulations Photos does not exist');
        }
    }
