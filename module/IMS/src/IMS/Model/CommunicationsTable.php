...
class CommunicationsTable extends AbstractTableGateway {

    protected $table_name = 'communications';
    ...
    public function getCommunicationsByCCL($company,$country,$location)
    {
    ...
    public function getCommunicationsByCCLType($company,$country,$location,$type)
    {
        $row = $this->select(function (Select $select) use ($company,$country,$location,$type){
            $select->where(array('company'=>(string) $company,
                                'country'=>(string) $country,
                                'location'=>(string) $location,
                                'type_comm'=>(string) $type,
                                'status'=>'A'));
            $select->order('date_creation DESC');
        });
        if (!$row)
    ...
    public function getCommunicationByCCLId($company,$country,$location,$id)
    {
    ...
    public function save(Communications $object)
    {
        $data = array(
        ...
        if (!$this->getCommunicationByCCLId($company,$country,$location,$id)) {
            ...
            return $data['id'];
        } elseif ($this->getCommunicationByCCLId($company,$country,$location,$id)) {
    ...
    public function updateCommunication($company,$country,$location,$id,$data)
    {
