...
use IMS\Model\Entity\ProcessThreadRelations;

class ProcessThreadView extends AbstractTableGateway {

    protected $table_name = 'process_thread_view';
    ...
    public function getSubProcessValue($id_subprocess,$id_process,$lang)
    {
        $row = $this->select(array('id'=>(int) $id_subprocess,'parent_id'=>(int) $id_process,'lang' => (string) $lang));
        if (!$row)
            return false;
        return $row;
    }
    
    public function getSubProcessUnassigned($lang) {
        $row = $this->select(function (Select $select) use ($lang) {
            $select->where(array('lang' => (string) $lang, 'assignment'=>'U'))->order('ordering ASC');
        });
        if (!$row)
            return false;
        $listItems=array();
