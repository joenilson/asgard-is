...
class MessagesTable extends AbstractTableGateway

{
    ...
    public function fetchAll() {
        $resultSet = $this->select(function (Select $select) {
            $select->order('lang,key ASC');
        });
        return $resultSet->toArray();
    }

    public function getMessage($lang_id,$key_id)
    {
        //$id  = (int) $id;
        $rowset = $this->select(array('lang' => $lang_id, 'key'=>$key_id));
        $row = $rowset->current();
        if (!$row) {
            throw new \Exception("Could not find row $lang_id or $key_id");
        }
        return $row;
    }
    ...
    */
    ...
    public function deleteMessage($lang_id,$key_id)
    {
        $this->tableGateway->delete(array('lang' => $lang_id, 'key'=>$key_id));
    }
}